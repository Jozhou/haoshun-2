...
public class ScreenManager {
    ...
    public final static ScreenManager get() {
        synchronized (mLock) {
            if (mInstance == null) {
...
//                android.os.Process.killProcess(android.os.Process.myPid());
     ...
     * @return
     */
    public synchronized boolean includeActivitys(Class ...targetClass) {
        if (activityStack != null) {
            for(int i = activityStack.size() - 1; i >= 0; i--) {
                WeakReference<Activity> reference= activityStack.get(i);
                if(reference != null) {
                    ...
                    if(context != null) {
                        for (int j = 0; j < targetClass.length; j++){
                            if(targetClass[j].isInstance(context)){
                                return true;
                            }
                        }
